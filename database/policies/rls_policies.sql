...
-- Only service role can insert/update restaurants (for admin operations)
...
-- Users can view their own entries and public entries from others
...
-- Users can view public reviews and their own reviews
...
-- Users can view responses to public reviews
...
-- Users can insert responses to reviews they can see
...
-- Users can update/delete their own responses
...
-- Users can insert votes on public reviews
...
-- Users can view follows involving themselves or to public profiles
...
-- Users can only view and manage their own taste profile history
...
-- Users can only view and manage their own recommendations
...
-- All authenticated users can view food pairings
CREATE POLICY "Authenticated users can view food pairings" ON public.food_pairings
    FOR SELECT USING (auth.role() = 'authenticated');

-- Only service role can manage food pairings
...
BEGIN
    ...
    -- Friends-only profiles can be viewed by friends
    IF profile_privacy = 'friends' THEN
        RETURN public.is_friends_with(viewer_id, profile_id);
    END IF;
    
    -- Private profiles cannot be viewed by others
    RETURN FALSE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;